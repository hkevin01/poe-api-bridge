...
class TestMessageInjection:
    ...
    def test_inject_tools_no_existing_system_message(
        self, handler, sample_tools, sample_messages
    ):
        """Test injecting tools when no system message exists."""
        enhanced = handler._inject_tools_into_messages(
            sample_messages, sample_tools, "auto"
        )
    ...
    def test_inject_tools_with_different_tool_choices(
        self, handler, sample_tools, sample_messages
    ):
        """Test tool injection with different tool_choice values."""
        # Test 'none'
        enhanced = handler._inject_tools_into_messages(
            sample_messages, sample_tools, "none"
        )
        assert "FORBIDDEN from using any tools" in enhanced[0].content

        # Test 'required'
        enhanced = handler._inject_tools_into_messages(
            sample_messages, sample_tools, "required"
        )
        ...
        tool_choice = {"type": "function", "function": {"name": "get_weather"}}
        enhanced = handler._inject_tools_into_messages(
            sample_messages, sample_tools, tool_choice
        )
...
class TestStreamingToolCallFiltering:
    ...
    async def test_streaming_tool_call_detection(self, handler):
        ...
        # Test the streaming
        result_chunks = []
        async for chunk in handler._stream_tool_aware_response(
            "test-model", mock_messages, "test-key", MagicMock()
        ):
        ...
        content_chunks = [
            chunk for chunk in result_chunks if "I'll help you with that." in chunk
        ]
        ...
        raw_xml_chunks = [
            ...
            if "<tool_call>" in chunk and "tool_calls" not in chunk
        ]
    ...
    def test_partial_tool_call_handling(self, handler):
        ...
        for partial in partial_states:
            ...
            assert (
                len(tool_calls) == 0
            ), f"Partial state '{partial}' should not parse as complete tool call"
