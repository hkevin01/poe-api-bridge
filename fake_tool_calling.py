...
class FakeToolCallHandler:
    ...
    async def process_request(self, request, api_key: str) -> Union[Dict[str, Any], StreamingResponse]:
        """
        ...
        if request.stream:
            ...
            return await self._generate_tool_aware_response(model, poe_messages, api_key, request)
    
    def _inject_tools_into_messages(self, messages: List, tools: List[Dict], tool_choice: Optional[Union[str, Dict]]) -> List:
...
"""
        ...
        if messages:
            ...
            system_msg = type(first_msg)(**{**first_msg.__dict__, 'role': "system", 'content': combined_system_content})
        else:
            # Fallback if no messages (shouldn't happen)
            from server import ChatCompletionMessage
            system_msg = ChatCompletionMessage(role="system", content=combined_system_content)
    ...
    async def _generate_tool_aware_response(self, model: str, messages: List[fp.ProtocolMessage], api_key: str, request) -> Dict[str, Any]:
        ...
        if isinstance(response_content, dict):
            ...
            response_text = response_content.get("content", "") or response_content.get("text", "") or str(response_content)
        else:
    ...
    async def _stream_tool_aware_response(self, model: str, messages: List[fp.ProtocolMessage], api_key: str, request) -> Iterator[str]:
                        ...
                        if chunk_data and chunk_data != "[DONE]":
                            try:
                                ...
                                if content_delta:
                                    ...
                                    while i < len(content_delta):
                                        ...
                                        if not in_tool_call:
                                            ...
                                            if "<tool_call>" in temp_buffer:
                                                ...
                                                start_pos = temp_buffer.rfind("<tool_call>")
                                                # Keep content before tool call
                                                before_tool = temp_buffer[:start_pos]
                                                if len(before_tool) > len(content_buffer) - len(content_delta):
                                                    # This content was in the current delta
                                                    chars_before = start_pos - (len(content_buffer) - len(content_delta))
                                                    if chars_before > 0:
                                                        filtered_content += content_delta[:chars_before]
                                                
                                                in_tool_call = True
                                                tool_call_buffer = temp_buffer[start_pos:]
                                                break
                                            else:
                                            ...
                                            if tool_call_buffer.endswith("</tool_call>"):
                                                # Tool call complete
                                                _, tool_calls = self._parse_tool_calls(tool_call_buffer)
                                    ...
                                    if not in_tool_call and filtered_content != content_delta:
                                        parsed["choices"][0]["delta"]["content"] = filtered_content
                                        modified_chunk = f"data: {json.dumps(parsed)}\n\n"
