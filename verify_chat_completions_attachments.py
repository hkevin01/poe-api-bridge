...
def main():
    ...
    response = client.chat.completions.create(
        model="Imagen-3-Fast",
        messages=[
            {"role": "user", "content": "Generate an image of a red cat sitting on a blue chair"}
        ],
        stream=False
    )
    ...
    if "https://pfst.cf2.poecdn.net" in content or ("http" in content and ("jpg" in content or "png" in content or "webp" in content)):
        print("✅ Response appears to contain an attachment URL - message.attachment handling working!")
    else:
    ...
    stream = client.chat.completions.create(
        model="Imagen-3-Fast", 
        messages=[
            {"role": "user", "content": "Create a simple drawing of a house"}
        ],
        stream=True
    )
    ...
    if "https://pfst.cf2.poecdn.net" in full_content or ("http" in full_content and ("jpg" in full_content or "png" in full_content or "webp" in full_content)):
        print("✅ Streaming response contains attachment URL - message.attachment streaming working!")
    else:
